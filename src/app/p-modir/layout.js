...
async function ModirLayout({ children }) {
  ...
  if (userIsExists) {
    try {
      const foundedModirUnit = await modirUnitModel
        .find(
          {
            $and: [{ User: JSON.parse(JSON.stringify(userIsExists))._id }],
          },
          "-__v "
        )
        .populate([
      ...
      if (!foundedModirUnit) {
        ...
        foundedModirUnit.map((prev) =>
          units.push({
            ...JSON.parse(JSON.stringify(prev.Unit)),
  ...
  return (
    <div className="flex relative w-full ">
      <UserProvider
        ...
        modirFounded={JSON.parse(JSON.stringify(modir))}
        unitsFounded={JSON.parse(JSON.stringify(units))}
      >
        <div>
          <Sidebar />
        </div>
