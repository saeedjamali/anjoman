...
export async function GET(req, { params }) {
    ...
    try {
        ...
        if (!year) {
            ...
            return Response.json({ message: "اطلاعات با موفقیت دریافت شد", status: 201, pricelist, len });
        }
        if (year && !grade && !gender) {
            const pricelist = await priceListModel.find({ year });
            const len = pricelist.length
            return Response.json({ message: "اطلاعات با موفقیت برای سال تحصیلی جاری دریافت شد", status: 201, pricelist, len });
        }

        if (gender == 3) {
            const pricelist = await priceListModel.find({ $and: [{ year }, { grade }] });
            const len = pricelist.length
            return Response.json({ message: "اطلاعات با موفقیت براساس سال تحصیلی ، مقطع و جنسیت دریافت شد", status: 201, pricelist, len });
        ...
        return Response.json({ message: "اطلاعات متناسب با این دوره و جنسیت یافت نشد", status: 401, pricelist, len });
