...
function AdminInformation({ admin, region }) {
    ...
    const submitHandler = async (event) => {
        ...
        try {
            if (admin.name == nameInp && admin.prsCode == prsCodeInp && admin.meliCode == meliCodeInp) {
            ...
            const response = await fetch("/api/admin/update", {
                method: "PUT",
                body: JSON.stringify({
                    user: admin.user, prsCode: prsCodeInp, meliCode: meliCodeInp, name: nameInp
                }),
                headers: {
                    ...
                    <div className='w-full p-2'>
                        <form >
                            <div className='grid grid-cols-1 md:grid-cols-2  md:gap-4'>
                                ...
                                <div className='relative mt-2 flex justify-end col-span-1'>
                                    <span className='text-[10px] absolute bg-slate-200 p-1 rounded-md left-2  w-24'>منطقه</span>
                                    <input className='input-text-information mt-2' value={`${region.regionName} - ${region.regionCode}`} disabled type='text' placeholder='منطقه' ></input>
                                </div>
                                <div className='relative mt-2 flex justify-end col-span-1'>
                                    <span className='text-[10px] absolute bg-slate-200 p-1 rounded-md left-2  w-24'>عنوان پست</span>
                                    <input className='input-text-information mt-2' value={levelInp == 1 ? 'کارشناس منطقه' : levelInp == 2 ? 'کارشناس استان' : levelInp == 3 ? 'کارشناس ستاد' : 'مدیر سیستم'} disabled type='text' placeholder='عوان پست' ></input>
                                </div>
