...
export default function ExcelCompanyUploader() {
  ...
  const addCompanyFromExcel = async (event) => {
    ...
    try {
      ...
      const response = await fetch("/api/manager/company/addfromexcel", {
        method: "POST",
        headers: {
      ...
      const addCompanies = await response.json();
      if (addCompanies.status == 201) {
        toast.success(addCompanies.message);
      } else {
          ...
          <div className="relative flex justify-end items-center gap-2">
            <Dropdown>
              ...
              <DropdownMenu
                ...
                onAction={(key) => console.log(company)}>
    ...
    return (
      <div className="flex flex-col gap-4">
        <div className="flex flex-col text-[12px] bg-purple-600 p-4 text-white rounded-lg mt-4">
          ...
          <span>بروزرسانی براساس کد یکتا شرکت(مندرج در فایل اکسل مستندات)  انجام می شود</span>
        ...
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <Button color="secondary" onClick={(event) => addCompanyFromExcel(event)} isLoading={isLoading}>
              ارسال به پایگاه
            </Button>
          </div>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">کل {companies.length} شرکت</span>
  ...
  return (
    ...
    <>
      ...
      >
        ...
        <TableBody emptyContent={" شرکتی یافت نشد"} items={sortedItems}>
          {(item) => (
