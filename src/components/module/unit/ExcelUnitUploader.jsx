...
const INITIAL_VISIBLE_COLUMNS = ["regionCode", "regionName", "schoolCode", "schoolName", "year"];
...
export default function ExcelUnitUploader() {
  ...
  const addUnitFromExcel = async (event) => {
    ...
    try {
      setIsLoading(true)
      const response = await fetch("/api/manager/unit/addfromexcel", {
        method: "POST",
        headers: {
      ...
      const addUnits = await response.json();
      if (addUnits.status == 201) {
        toast.success(addUnits.message);
      } else {
          ...
          <div className="relative flex justify-end items-center gap-2">
            <Dropdown>
              ...
              <DropdownMenu
                ...
                onAction={(key) => console.log(school)}>
  ...
  const topContent = React.useMemo(() => {
    return (
      <div className="flex flex-col gap-4">
        <div className="flex flex-col text-[12px] bg-purple-600 p-4 text-white rounded-lg">
          <span className="underline font-iranyekanBold">نکات بروزرسانی:</span>
          <span>بروزرسانی براساس کد واحد سازمانی و سال تحصیلی انجام می شود</span>
          <span>در صورتی که واحد سازمانی - سال تحصیل از قبل وجود داشته باشد فیلد های تعداد دانش آموز دختر - پسر و طول و عرض جغرافیایی بروز می شود.</span>
        </div>
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <Button color="secondary" onClick={(event) => addUnitFromExcel(event)} isLoading={isLoading}>
              ارسال به پایگاه
            </Button>
          </div>
        </div>
  ...
  return (
    ...
    <div className="mt-4">
      ...
      >
        ...
        <TableBody emptyContent={"مدرسه ای یافت نشد"} items={sortedItems}>
          {(item) => (
            <TableRow key={item.schoolCode}>
