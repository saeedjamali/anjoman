...
const INITIAL_VISIBLE_COLUMNS = ["provinceName", "provinceCode", "regionCode", "regionName", "accessType"];
...
export default function ExcelRegionUploader() {
    ...
    const addRegionFromExcel = async (event) => {
        ...
        try {
            ...
            const response = await fetch("/api/manager/region/addfromexcel", {
                method: "POST",
                headers: {
            ...
            if (addRegion.status == 201) {
                toast.success(addRegion.message);

            } else {
                toast.error(addRegion.message);
            }
        } catch (error) {
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredRegion = filteredRegion.filter((item) =>
                item?.provinceName?.toLowerCase().includes(filterValue.toLowerCase()) ||
                item?.regionName?.toLowerCase().includes(filterValue.toLowerCase()) ||
                item?.provinceCode == filterValue ||
                item?.regionCode == filterValue,
            );
        }
    ...
    }, [region, filterValue, statusFilter]);
    ...
    const renderCell = React.useCallback((item, columnKey) => {
        ...
        switch (columnKey) {
            ...
            //! در افزودن از طریق اکسل نیاز به اکشن نداریم
            case "actions":
                return (
        ...
        return (
            <div className="flex flex-col gap-4">
                <div className="flex flex-col text-[12px] bg-purple-600 p-4 text-white rounded-lg mt-4">
                    ...
                    <span>بروزرسانی براساس کد منطقه شرکت(مندرج در فایل اکسل مستندات)  انجام می شود</span>
                ...
                <div className="flex justify-between gap-3 items-end">
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button color="secondary" onClick={(event) => addRegionFromExcel(event)}>
                            <div className="flex-1 flex">ارسال به پایگاه</div>
                ...
                <div className="flex justify-between items-center">
                    <span className="text-default-400 text-small">کل {region.length} شرکت</span>
    ...
    return (
        ...
        <>
            ...
            >
                ...
                <TableBody emptyContent={" منطقه ای یافت نشد"} items={sortedItems}>
                    {(item) => (
                        <TableRow key={item.regionCode}>
